...
class IOSScreen(BaseScreen):
    def __init__(self):
        super().__init__("Apple iOS", "Gaming stuff giveaways available for iOS.")
        ...
        self.display_area.setHtml(
            giveaways_default_screen_html.html("https://upload.wikimedia.org/wikipedia/commons/6/63/IOS_wordmark_%282017%29.svg", "iOS")
        )
    ...
    def fetchAllGiveaways(self):
        ...
        self.fetcher = giveaways.GamesAndLoots(platform="ios", cache_data_path="./data/ios_gaming_stuff_giveaways.json")
    ...
    def fetchGamesGiveaways(self):
        from .html import loading_screen
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(platform="ios", loots=False, cache_data_path="./data/ios_gaming_stuff_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def fetchLootsGiveaways(self):
        from .html import loading_screen
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(platform="ios", games=False, cache_data_path="./data/ios_gaming_stuff_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def open_save_to_db_dialog(self):
        from ...cache_data_files import ios_gaming_stuff_giveaways
        if not os.path.exists(ios_gaming_stuff_giveaways):
            # check ./data dir if not exist then create one
            os.makedirs("./data", exist_ok=True)
            # create ./data/ios_gaming_stuff_giveaways.json file
            with open("./data/ios_gaming_stuff_giveaways.json", "w") as file:
                file.write("[]")
                
            # informational message box
            title = "Data cache file empty!"
            message = "newly ./data/ios_gaming_stuff_giveaways.json just created because, data cache not found! So, please refetch data then try again to insert data into Database."
        ...
        db_dialog = SaveToDBDialog.DBDialog(ios_gaming_stuff_giveaways)
        db_dialog.exec()