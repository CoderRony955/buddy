...
class XboxScreen(BaseScreen):
    def __init__(self):
        ...
        self.display_area.setHtml(
            giveaways_default_screen_html.html("https://upload.wikimedia.org/wikipedia/commons/8/84/Xbox_logo_2012_cropped.svg", "Xbox")
        )
    ...
    def fetchAllGiveaways(self):
        ...
        self.fetcher = giveaways.GamesAndLoots(platform="xbox-series-xs", cache_data_path="./data/xbox_gaming_stuff_giveaways.json")
    ...
    def fetchGamesGiveaways(self):
        from .html import loading_screen
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(platform="xbox-series-xs", loots=False, cache_data_path="./data/xbox_gaming_stuff_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def fetchLootsGiveaways(self):
        from .html import loading_screen
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(platform="xbox-series-xs", games=False, cache_data_path="./data/xbox_gaming_stuff_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def open_save_to_db_dialog(self):
        ...
        if not os.path.exists(xbox_gaming_stuff_giveaways):
            ...
            # create ./data/xbox_gaming_stuff_giveaways.json file
            with open("./data/xbox_gaming_stuff_giveaways.json", "w") as file:
                file.write("[]")
                
            # informational message box
            title = "Data cache file empty!"
            message = "newly ./data/xbox_gaming_stuff_giveaways.json just created because, data cache not found! So, please refetch data then try again to insert data into Database."
