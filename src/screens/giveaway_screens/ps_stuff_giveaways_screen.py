...
class PlayStationScreen(BaseScreen):
    def __init__(self):
        super().__init__("PlayStation 5", "Giveaways available for PlayStation 5.")
        ...
        self.display_area.setHtml(
            giveaways_default_screen_html.html("https://upload.wikimedia.org/wikipedia/commons/8/82/Cib-playstation_%28CoreUI_Icons_v1.0.0%29.svg", "PlayStation")
        )
    ...
    def fetchAllGiveaways(self):
        ...
        self.fetcher = giveaways.GamesAndLoots(platform="ps5", cache_data_path="./data/playstation_gaming_stuff_giveaways.json")
    ...
    def fetchGamesGiveaways(self):
        from .html import loading_screen
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(platform="ps5", loots=False, cache_data_path="./data/playstation_gaming_stuff_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def fetchLootsGiveaways(self):
        from .html import loading_screen
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(platform="ps5", games=False, cache_data_path="./data/playstation_gaming_stuff_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def open_save_to_db_dialog(self):
        from ...cache_data_files import xbox_gaming_stuff_giveaways
        if not os.path.exists(xbox_gaming_stuff_giveaways):
            # check ./data dir if not exist then create one
            os.makedirs("./data", exist_ok=True)
            # create ./data/playstation_gaming_stuff_giveaways.json file
            with open("./data/playstation_gaming_stuff_giveaways.json", "w") as file:
                file.write("[]")
                
            # informational message box
            title = "Data cache file empty!"
            message = "newly ./data/playstation_gaming_stuff_giveaways.json just created because, data cache not found! So, please refetch data then try again to insert data into Database."
        ...
        db_dialog = SaveToDBDialog.DBDialog(xbox_gaming_stuff_giveaways)
        db_dialog.exec()