...
class SwitchScreen(BaseScreen):
    def __init__(self):
        super().__init__("Nintendo Switch", "Free Gaming stuff Giveaways for Nintendo Switch.")
        ...
        self.display_area.setHtml(
            giveaways_default_screen_html.html("https://upload.wikimedia.org/wikipedia/commons/3/3f/Nintendo_Switch_Logo_%28without_text%29.svg", "Nintendo Switch")
        )
    ...
    def fetchAllGiveaways(self):
        ...
        self.fetcher = giveaways.GamesAndLoots(platform="switch", cache_data_path="./data/switch_gaming_stuff_giveaways.json")
    ...
    def fetchGamesGiveaways(self):
        from .html import loading_screen
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(platform="switch", loots=False, cache_data_path="./data/switch_gaming_stuff_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def fetchLootsGiveaways(self):
        from .html import loading_screen
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(platform="switch", games=False, cache_data_path="./data/switch_gaming_stuff_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def open_save_to_db_dialog(self):
        from ...cache_data_files import switch_gaming_stuff_giveaways
        if not os.path.exists(switch_gaming_stuff_giveaways):
            # check ./data dir if not exist then create one
            os.makedirs("./data", exist_ok=True)
            # create ./data/switch_gaming_stuff_giveaways.json file
            with open("./data/switch_gaming_stuff_giveaways.json", "w") as file:
                file.write("[]")
                
            # informational message box
            title = "Data cache file empty!"
            message = "newly ./data/switch_gaming_stuff_giveaways.json just created because, data cache not found! So, please refetch data then try again to insert data into Database."
        ...
        db_dialog = SaveToDBDialog.DBDialog(switch_gaming_stuff_giveaways)
        db_dialog.exec()