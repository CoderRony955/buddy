...
class EpicGamesScreen(BaseScreen):
    def __init__(self):
        super().__init__("Epic Games",
                         "Free-to-play titles & loots currently tracked from the Epic Games store.")
        layout = self.layout()
        hlayout = QHBoxLayout()
        ...
        game_giveaways_fetch_btn.setIcon(
            QIcon(qta.icon("fa5s.gamepad", style="white")))
        ...
        gaming_loot_giveaways_fetch_btn.setIcon(
            QIcon(qta.icon("fa5s.box-open", style="white")))
        gaming_loot_giveaways_fetch_btn.setToolTip(
            "fetch latest gaming loot giveaways")
        gaming_loot_giveaways_fetch_btn.setCursor(
        ...
        gaming_loot_giveaways_fetch_btn.clicked.connect(
            self.fetchLootsGiveaways)
        ...
        self.display_area.setHtml(
            giveaways_default_screen_html.html(
                "https://upload.wikimedia.org/wikipedia/commons/3/31/Epic_Games_logo.svg", "Epic Games")
        )
    ...
    def fetchAllGiveaways(self):
        ...
        self.fetcher = giveaways.GamesAndLoots(
            platform="epic-games-store", cache_data_path="./data/epic_games_giveaways.json")
    ...
    def fetchGamesGiveaways(self):
        ...
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(
            platform="epic-games-store", loots=False, cache_data_path="./data/epic_games_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def fetchLootsGiveaways(self):
        ...
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(
            platform="epic-games-store", games=False, cache_data_path="./data/epic_games_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def open_save_to_db_dialog(self):
        from ...cache_data_files import epic_games_gaming_stuff_giveaways
        if not os.path.exists(epic_games_gaming_stuff_giveaways):
            # check ./data dir if not exist then create one
            os.makedirs("./data", exist_ok=True)
            # create ./data/epic_games_giveaways.json file
            with open("./data/epic_games_giveaways.json", "w") as file:
                file.write("[]")

            # informational message box
            title = "Data cache file empty!"
            message = "newly ./data/epic_games_giveaways.json just created because, data cache not found! So, please refetch data then try again to insert data into Database."
        ...
        db_dialog = SaveToDBDialog.DBDialog(epic_games_gaming_stuff_giveaways)
        db_dialog.exec()
