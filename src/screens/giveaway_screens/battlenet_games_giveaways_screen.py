...
class BattlenetScreen(BaseScreen):
    def __init__(self):
        ...
        self.display_area.setHtml(
            giveaways_default_screen_html.html("https://www.svgrepo.com/show/313544/battle-net.svg", "Battle net")
        )
    ...
    def fetchAllGiveaways(self):
        ...
        self.fetcher = giveaways.GamesAndLoots(platform="battlenet", cache_data_path="./data/battlenet_giveaways.json")
    ...
    def fetchGamesGiveaways(self):
        from .html import loading_screen
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(platform="battlenet", loots=False, cache_data_path="./data/battlenet_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def fetchLootsGiveaways(self):
        from .html import loading_screen
        self.display_area.setHtml(loading_screen.html)
        self.fetcher = giveaways.GamesAndLoots(platform="battlenet", games=False, cache_data_path="./data/battlenet_giveaways.json")
        self.fetcher.giveaways.connect(self.display_giveaways)
        self.fetcher.start()
    ...
    def open_save_to_db_dialog(self):
        from ...cache_data_files import battlenet_gaming_stuff_giveaways
        if not os.path.exists(battlenet_gaming_stuff_giveaways):
            # check ./data dir if not exist then create one
            os.makedirs("./data", exist_ok=True)
            # create ./data/battelnet_giveaways.json file
            with open("./data/battelnet_giveaways.json", "w") as file:
                file.write("[]")
                
            # informational message box
            title = "Data cache file empty!"
            message = "newly ./data/battelnet_giveaways.json just created because, data cache not found! So, please refetch data then try again to insert data into Database."
        ...
        db_dialog = SaveToDBDialog.DBDialog(battlenet_gaming_stuff_giveaways)
        db_dialog.exec()
